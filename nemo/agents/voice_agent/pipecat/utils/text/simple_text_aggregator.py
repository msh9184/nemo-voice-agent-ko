...
class SimpleSegmentedTextAggregator(SimpleTextAggregator):
    ...
    async def reset(self):
        """Reset the aggregator state, clearing any accumulated text.

        Note: This method must be async because pipecat's TTSService calls
        `await self._text_aggregator.reset()` when processing LLMFullResponseEndFrame.
        """
        self._text = ""
