...
class NemoDiarService(STTService):
    ...
    async def _handle_diarization_result(self, diar_result):
        ...
        try:
            if diar_result is None:
                logger.debug("DIAR | No result returned")
                return

            dominant_speaker_id = self._get_dominant_speaker_id(diar_result)

            # Log diarization processing - clean format
            if dominant_speaker_id is None:
                logger.debug(f"DIAR | No speaker (threshold={self._params.threshold})")
            elif dominant_speaker_id == self._current_speaker_id:
                # Same speaker - only debug log
                pass
            else:
                # New speaker detected - push frame
                old_speaker = self._current_speaker_id
                self._current_speaker_id = dominant_speaker_id
                # Clean speaker change log
                old_label = f"S{old_speaker + 1}" if old_speaker is not None else "-"
                new_label = f"S{dominant_speaker_id + 1}"
                logger.info(f"DIAR | Speaker: {old_label} -> {new_label}")
                await self.push_frame(DiarResultFrame(dominant_speaker_id, stream_id="default"))
        except Exception as e:
            logger.error(f"DIAR | Error: {e}")
    ...
    def _get_dominant_speaker_id(self, spk_pred: np.ndarray):
        ...
        # IMPORTANT: Convert numpy int64 to Python int for isinstance() checks downstream
        # np.argmax() and numpy array indexing return np.int64, which fails isinstance(x, int)
        if dominant_speaker_id is not None:
            dominant_speaker_id = int(dominant_speaker_id)
