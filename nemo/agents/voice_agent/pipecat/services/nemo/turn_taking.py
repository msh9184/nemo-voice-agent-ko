...
class NeMoTurnTakingService(FrameProcessor):
    ...
    async def _handle_transcription(
        ...
        if self._vad_user_speaking:
            # NOTE: Streaming ASR sends complete accumulated hypothesis, not incremental text.
            # Replace buffer instead of appending to avoid duplicate accumulation.
            ...
            if is_backchannel and self._bot_speaking:
                ...
                # if the text segment is not empty and have non-space characters, replace the buffer
                # NOTE: Streaming ASR sends complete accumulated hypothesis, not incremental text.
                self._user_speaking_buffer = text_segment
                if self.is_backchannel(self._user_speaking_buffer):
                    ...
                    self._have_sent_user_started_speaking = False
                    return
                logger.debug(f"Updated user speaking buffer: `{self._user_speaking_buffer}`")
    ...
    async def _handle_completed_text(self, completed_text: str, direction: FrameDirection, is_final: bool = True):
        ...
        # Clean text: remove EOU/EOB tokens and SentencePiece artifacts
        ...
        completed_text = completed_text.replace("\u2581", " ")  # ▁ -> space
        # Remove Unicode replacement characters
        completed_text = completed_text.replace("\uFFFD", "")  # � (replacement character)
        # Collapse multiple spaces into one
        import re
        completed_text = re.sub(r'\s+', ' ', completed_text).strip()
