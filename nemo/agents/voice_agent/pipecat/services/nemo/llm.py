...
class HuggingFaceLLMLocalService(LLMUtilsMixin):
    ...
    async def generate_stream(
        ...
        # Remove token_type_ids if the model doesn't use them
        # Most decoder-only LLMs (GPT, LLaMA, Qwen, Mistral, etc.) don't use token_type_ids
        # Only encoder models like BERT use them for segment distinction
        if "token_type_ids" in inputs:
            # Check if model actually uses token_type_ids by checking type_vocab_size
            # type_vocab_size > 0 means the model uses token_type_ids (e.g., BERT)
            # type_vocab_size == 0 or not present means decoder-only model
            model_config = self.model.config
            uses_token_type_ids = getattr(model_config, 'type_vocab_size', 0) > 0

            if not uses_token_type_ids:
                del inputs["token_type_ids"]
                logger.debug(f"Removed token_type_ids (model {model_config._name_or_path} doesn't use them)")
