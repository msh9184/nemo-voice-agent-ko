...
class WebsocketServerInputTransport(BaseInputTransport):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # JSON message handler for RTVI protocol messages
        self._json_message_handler = None

    def set_json_message_handler(self, handler):
        """Set a handler for incoming JSON text messages.

        Args:
            handler: Async function that takes a JSON string message
        """
    ...
    async def _client_handler(self, websocket: websockets.WebSocketServerProtocol, path: Optional[str] = None):
        ...
        try:
            async for message in websocket:
                # Check if message is JSON text (RTVI protocol) or binary (Protobuf frames)
                if isinstance(message, str):
                    # JSON text message - likely RTVI protocol message
                    # These are handled by the RTVI processor, not the frame pipeline
                    # Store for external handlers to process
                    if hasattr(self, '_json_message_handler') and self._json_message_handler:
                        try:
                            await self._json_message_handler(message)
                        except Exception as e:
                            logger.debug(f"JSON message handler error: {e}")
                    else:
                        logger.debug(f"Received JSON message (no handler): {message[:100]}...")
                    continue

                # Binary message - use serializer for Protobuf frames
                ...
                if isinstance(frame, InputAudioRawFrame):
                    logger.debug(f"Received InputAudioRawFrame: {len(frame.audio)} bytes, sample_rate={frame.sample_rate}")
...
class WebsocketServerTransport(BaseTransport):
    ...
    def set_json_message_handler(self, handler):
        """Set a handler for incoming JSON text messages on the input transport.

        This allows processing RTVI protocol messages that come as JSON strings
        instead of binary Protobuf frames.

        Args:
            handler: Async function that takes a JSON string message
        """
        if self._input:
            self._input.set_json_message_handler(handler)
        else:
            # Create input transport first if needed
            self.input().set_json_message_handler(handler)
