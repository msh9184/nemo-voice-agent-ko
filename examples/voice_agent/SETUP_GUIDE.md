...
flowchart TD
    subgraph GPU["GPU Server"]
      direction LR
      STT["STT"] --> DIAR["Diarization (Sortformer)"]
      DIAR --> LLM["LLM (Nemotron)"]
      LLM --> TTS["TTS (Kokoro)"]
    end
    GPU -->|"HTTP :7860 / WebSocket :8765"| NET["Network (LAN)"]
    NET --> PC["Client PC - Web Browser (:5173)<br/>Microphone - Speaker - RTVI client"]
```
